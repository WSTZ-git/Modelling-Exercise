% 注意开始书写之前一定要先调整本文件第3行和第35行的文件路径!!!!!!!!!!
% 注意这里引用的模板路径需要根据main.tex文件的具体位置确定(文件位置不同路径中的"../"的个数不同)!!!
...
% 载入路径
% 让主文件或子文件在引用图片、输入其他 tex 文件时，不用写完整路径，LaTeX 会自动在这些路径下查找目标文件，方便管理和引用。
\makeatletter
...
% 注意这里引用的宏包路径需要根据main.tex文件的具体位置确定(文件位置不同路径中的"../"的个数不同)!!!
\usepackage{subfiles}
